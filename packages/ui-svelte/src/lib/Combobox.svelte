<script lang="ts">
	import {createComboboxController, type ComboboxLoadOptions, type ComboboxState, type SelectOption} from '@ooopsstudio/ui-primitives'
	import {onMount, type Snippet} from 'svelte'
	import {portal} from './portal'
	type Props = {id?: string; name?: string; label?: string; description?: string; value?: string; options?: SelectOption[]; placeholder?: string; allowCustomValue?: boolean; disabled?: boolean; required?: boolean; clearable?: boolean; portal?: boolean; loadOptions?: ComboboxLoadOptions; debounceMs?: number; class?: string; option?: Snippet<[SelectOption, {selected: boolean; active: boolean}]>; onChange?: (detail: {value: string; option: SelectOption | null; custom: boolean}) => void}
	const generatedId = $props.id()
	let {id = generatedId, name, label, description, value = $bindable(''), options = [], placeholder, allowCustomValue = false, disabled = false, required = false, clearable = true, portal: usePortal = true, loadOptions, debounceMs = 150, class: className = '', option, onChange}: Props = $props()
...
<div bind:this={root} class={`ooops-combobox ${className}`.trim()} data-part="root" data-state={state.open ? 'open' : 'closed'} data-loading={state.loading ? 'true' : 'false'}>
	...
	<div data-part="control">
		<input bind:this={input} id={`${id}-input`} role="combobox" aria-autocomplete="list" aria-controls={`${id}-listbox`} aria-expanded={state.open} aria-labelledby={label ? `${id}-label` : undefined} aria-describedby={description ? `${id}-description` : undefined} {placeholder} {disabled} {required} data-part="input" />
		{#if clearable}<button type="button" aria-label="Clear selection" data-part="clear" disabled={disabled || !state.value} onclick={() => controller?.setValue('', true)}>×</button>{/if}
...
<div use:portal={usePortal} bind:this={listbox} id={`${id}-listbox`} role="listbox" aria-labelledby={label ? `${id}-label` : undefined} hidden={!state.open} data-part="listbox">
	{#if state.loading}<div role="status" data-part="loading">Loading…</div>{/if}
	{#if state.error}<div role="alert" data-part="error">{state.error}</div>{/if}
	{#if !state.loading && !state.error && state.options.length === 0}<div data-part="empty">No options</div>{/if}
