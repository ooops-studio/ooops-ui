<script lang="ts">
	...
	type Props = {id?: string; name?: string; label?: string; description?: string; hint?: string; error?: string; value?: string; placeholder?: string; required?: boolean; disabled?: boolean; rows?: number; maxlength?: number; autoResize?: boolean; minRows?: number; maxRows?: number; showCount?: boolean; class?: string; onChange?: (value: string) => void}
	const generatedId = $props.id()
	let {id = generatedId, name, label, description, hint, error, value = $bindable(''), placeholder, required = false, disabled = false, rows = 3, maxlength, autoResize = false, minRows, maxRows, showCount = false, class: className = '', onChange}: Props = $props()
...
<div class={`ooops-textarea ${className}`.trim()} data-part="root" data-invalid={error ? 'true' : 'false'} data-disabled={disabled ? 'true' : 'false'}>
	...
	<textarea bind:this={element} {id} {name} {placeholder} {required} {disabled} {rows} maxlength={maxlength} aria-describedby={describedBy} aria-invalid={error ? 'true' : undefined} data-part="control"></textarea>
