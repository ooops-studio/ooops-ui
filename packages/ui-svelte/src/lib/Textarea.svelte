<script lang="ts">
	...
	onMount(() => {
		controller = createTextareaController({value, defaultValue: value, autoResize, ...(minRows === undefined ? {} : {minRows}), ...(maxRows === undefined ? {} : {maxRows}), getElement: () => element, onValueChange: commitValue})
		controller.mount()
...
<div class={`ooops-textarea ${className}`.trim()} data-part="root" data-invalid={error ? 'true' : 'false'} data-disabled={disabled ? 'true' : 'false'}>
	...
	<textarea bind:this={element} bind:value {id} {name} {placeholder} {required} {disabled} {readonly} {rows} maxlength={maxlength} aria-describedby={describedBy} aria-invalid={error ? 'true' : undefined} data-part="control" oninput={(event) => commitValue(event.currentTarget.value)}></textarea>
