<script lang="ts">
	import {createTabsController, resolveUiMessages, type TabsState, type UiMessages} from '@ooopsstudio/ui-primitives'
	import {onMount, tick, type Snippet} from 'svelte'
	import type {TabItem} from './types.js'
	type Props = {id?: string; items: TabItem[]; activeId?: string; mode?: 'panels' | 'navigation'; activation?: 'automatic' | 'manual'; orientation?: 'horizontal' | 'vertical'; loop?: boolean; ariaLabel?: string; messages?: Partial<UiMessages>; class?: string; tab?: Snippet<[TabItem, boolean]>; panel?: Snippet<[TabItem]>; onChange?: (id: string) => void}
	const generatedId = $props.id()
	let {id = generatedId, items, activeId = $bindable(''), mode = 'panels', activation = 'automatic', orientation = 'horizontal', loop = true, ariaLabel, messages, class: className = '', tab, panel, onChange}: Props = $props()
	...
	onMount(() => { const initialActive = activeId || items.find((entry) => !entry.disabled)?.id || ''; activeId = initialActive; controller = createTabsController({activeId: initialActive, defaultActiveId: initialActive, mode, activation, orientation, loop, getTabs: () => tabs.filter(Boolean), getPanels: () => panels.filter(Boolean), onChange: (next) => { activeId = next; onChange?.(next) }}); const unsubscribe = controller.subscribe((next) => { state = next }); controller.mount(); return () => { unsubscribe(); controller?.destroy(); controller = null } })
	$effect(() => { if (controller && controller.getState().activeId !== activeId) controller.setActive(activeId) })
	$effect(() => controller?.configure({mode, activation, orientation, loop}))
	$effect(() => {
		items
...
<div {id} class={`ooops-tabs ${className}`.trim()} data-part="root" data-orientation={orientation} data-mode={mode}>
	<div role={mode === 'panels' ? 'tablist' : undefined} aria-label={mode === 'panels' ? ariaLabel ?? uiMessages.tabs : undefined} aria-orientation={mode === 'panels' ? orientation : undefined} data-part="list">
