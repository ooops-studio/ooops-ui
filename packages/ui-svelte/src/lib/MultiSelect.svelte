<script lang="ts">
	import {createMultiSelectController, type MultiSelectState, type SelectOption} from '@ooopsstudio/ui-primitives'
	import {onMount, type Snippet} from 'svelte'
	import {portal} from './portal'
	type Props = {id?: string; name?: string; label?: string; values?: string[]; options: SelectOption[]; placeholder?: string; disabled?: boolean; maxSelected?: number; portal?: boolean; class?: string; option?: Snippet<[SelectOption, boolean]>; chip?: Snippet<[SelectOption]>; onChange?: (values: readonly string[]) => void}
	const generatedId = $props.id()
	let {id = generatedId, name, label, values = $bindable([]), options, placeholder, disabled = false, maxSelected, portal: usePortal = true, class: className = '', option, chip, onChange}: Props = $props()
...
<div bind:this={root} class={`ooops-multi-select ${className}`.trim()} data-part="root" data-state={state.open ? 'open' : 'closed'} data-max-selected={maxSelected !== undefined && state.values.length >= maxSelected ? 'true' : 'false'}>
	...
	<div data-part="control">
		<div data-part="chips">{#each selected as entry (entry.value)}<span data-part="chip" data-value={entry.value}>{#if chip}{@render chip(entry)}{:else}{entry.label}{/if}<button type="button" data-part="chip-remove" data-remove-value={entry.value} aria-label={`Remove ${entry.label}`}>×</button></span>{/each}</div>
		<input bind:this={input} id={`${id}-input`} role="combobox" aria-controls={`${id}-listbox`} aria-expanded={state.open} aria-labelledby={label ? `${id}-label` : undefined} {placeholder} {disabled} data-part="input" />
		<button type="button" data-part="clear" aria-label="Clear selections" disabled={disabled || !state.values.length} onclick={() => controller?.clear()}>×</button>
	</div>
...
<div use:portal={usePortal} bind:this={listbox} id={`${id}-listbox`} role="listbox" aria-multiselectable="true" hidden={!state.open} data-part="listbox">
	<div data-part="actions"><button type="button" data-part="select-all" onclick={() => controller?.selectAll()}>Select all</button><button type="button" data-part="clear" onclick={() => controller?.clear()}>Clear</button></div>
	{#if state.options.length === 0}<div data-part="empty">No options</div>{/if}
