...
test('[FM-CHECKBOX-TRANSITIONS] indeterminate and disabled @critical', async({page}) => {
	const mixed = page.locator('#astro-mixed')
	await expect(mixed).toHaveAttribute('aria-checked', 'mixed')
	await mixed.check()
	await expect(mixed).toHaveAttribute('aria-checked', 'true')
	await expect(mixed.locator('xpath=ancestor::*[@data-part="root"]')).toHaveAttribute('data-state', 'checked')
	await expect(page.locator('#astro-disabled-check')).toBeDisabled()
})

test('[FM-MULTISELECT-BOUNDS] max/select-all/clear/chip removal @critical', async({page}) => {
	const input = page.locator('#astro-multi')
	await input.focus()
	const listbox = page.getByRole('listbox', {name: 'Tags'})
	await listbox.getByRole('button', {name: 'Select all'}).click()
	const chips = page.locator('[data-ooops-multi-select-root] [data-part="chip"]')
...
test('[FM-TOOLTIP-POINTER] hover delay, close delay, coarse pointer, and focus fallback work', async({page}) => {
	const trigger = page.locator('#astro-tooltip-delayed-trigger')
	const tooltip = page.getByRole('tooltip', {name: 'Delayed Astro context'})
	await trigger.hover()
	...
	await page.evaluate(() => {
		...
		window.matchMedia = (query: string) =>
			query === '(pointer: coarse)'
				...
				: original(query)
		document.querySelector('#astro-tooltip-delayed-trigger')
...
test('[FM-TABS-MODES] manual activation separates focus from selection and navigation uses links', async({page}) => {
	const manual = page.locator('#astro-tabs-manual')
	...
	const navigation = page.locator('#astro-navigation-tabs')
...
test('[FM-SLIDER-KEYBOARD] Home/End/Page/collision/RTL @critical', async({page}) => {
	const thumbs = page.locator('#astro-slider [data-part="thumb"]')
	...
	await expect(high).toHaveAttribute('aria-valuenow', '80')
	const rtl = page.locator('#astro-slider-rtl [data-part="thumb"]')
	await rtl.focus()
...
test('[FM-NUMBER-INTERMEDIATE] invalid intermediate text remains editable and finite values clamp on blur', async({page}) => {
	const input = page.locator('#astro-number')
	await input.fill('12')
...
test('[FM-TEXTAREA-GEOMETRY] auto-resize grows to max rows and then enables scrolling', async({page}) => {
	const textarea = page.locator('#astro-bio')
